...
const contentDir = path.join(root, "content", "level-2");
const tiers = JSON.parse(fs.readFileSync(path.join(root, "data", "level-2-term-tiers.json"), "utf8"));
...
if (lessons.length !== expectedCount) errors.push(`Standardized Level 2 lesson count must be ${expectedCount}, received ${lessons.length}.`);
...
if (errors.length > 0) {
  console.error("Level 2 terminology audit failed:\n");
...
console.log(
  `Level 2 terminology audit passed: ${lessons.length}/${expectedCount} lessons, ${assignmentCount}/${assignmentCount} classified term assignments, ${uniqueAssigned} unique glossary terms.`,
);

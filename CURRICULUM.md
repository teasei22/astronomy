# ASTRAEA Curriculum

更新日: 2026-08-15  
構造: Level → Course → Module → Lesson

## 学習経路

```text
L0 宇宙への入口
  ↓
L1 天文学の地図
  ↓
L2 観測する宇宙 + 必要時に学ぶ数学・物理
  ├──────────────┐
  ↓              ↓
L3 太陽・惑星   L4 恒星・星間物質
  └──────┬───────┘
         ↓
L5 銀河・極限宇宙
         ↓
L6 宇宙論
         ↓
L7 大学天文学・研究実践
```

数学・物理は別の長い関門にせず、各天文学上の問いから Bridge Lesson へ接続する。L7 では、それまでの断片を大学学部レベルの体系へ統合する。

---

## LEVEL 0 — 宇宙への入口

対象: 完全初心者。数式は原則使わない。

### Course 0A: 私たちの宇宙住所

- Module 0A-1 地球から宇宙へ
  - L0-01 宇宙は「どこから」始まるのか
  - L0-02 地球・月・太陽の大きさと距離
  - L0-03 太陽系はどこまで続くのか
- Module 0A-2 恒星から宇宙へ
  - L0-04 太陽も一つの恒星である
  - L0-05 天の川銀河の中の私たち
  - L0-06 銀河群・銀河団・宇宙の大規模構造

### Course 0B: 距離と時間の感覚

- Module 0B-1 宇宙のものさし
  - L0-07 km・天文単位 AU・光年
  - L0-08 parsec・kpc・Mpc・Gpc
  - L0-09 見かけの大きさと本当の大きさ
- Module 0B-2 光で過去を見る
  - L0-10 光には届くまでの時間がかかる
  - L0-11 宇宙年齢 138 億年のタイムライン
  - L0-12 観測可能な宇宙と「宇宙全体」の違い

### Course 0C: 最初の科学的な見方

- Module 0C-1 見ることから推理する
  - L0-13 宇宙から届く信号を読む
  - L0-14 観測・データ・モデル・結論
  - L0-15 どこまで確かに言えるのか
- Module 0C-2 最初の誤解をほどく
  - L0-16 光年は時間ではなく距離
  - L0-17 季節を決めるのは距離か
  - L0-18 ビッグバンはどこで起きたのか

---

## LEVEL 1 — 天文学の地図

対象: 分野の関係と「何をどう調べる学問か」を理解する。

### Course 1A: 天文学という科学

- Module 1A-1 三つの研究方法
  - L1-01 観測天文学: 宇宙から届く信号を測る
  - L1-02 理論天文学: 法則から予測を作る
  - L1-03 計算天文学: 複雑な宇宙を計算機で試す
- Module 1A-2 スケール別の分野
  - L1-04 惑星科学とアストロバイオロジー
  - L1-05 太陽物理と宇宙天気
  - L1-06 恒星・星間物質・高エネルギー天文学
  - L1-07 銀河天文学と宇宙論
  - L1-08 重力波・ニュートリノ・マルチメッセンジャー

### Course 1B: 宇宙観を変えた観測

- Module 1B-1 空の規則性から物理法則へ
  - L1-09 古代の暦と天球
  - L1-10 コペルニクス、ティコ、ケプラー
  - L1-11 ガリレオの望遠鏡とニュートンの重力
- Module 1B-2 光を分け、見えない宇宙を見る
  - L1-12 分光学が恒星を物理学へ変えた
  - L1-13 写真・電波・宇宙望遠鏡
  - L1-14 銀河の宇宙、CMB、系外惑星、重力波

### Course 1C: 現代の観測者たち

- Module 1C-1 光を集める施設
  - L1-15 Hubble・JWST・Subaru・VLT
  - L1-16 ALMA・Chandra と多波長観測
  - L1-17 Gaia・Kepler・TESS とサーベイ
- Module 1C-2 光以外の宇宙
  - L1-18 LIGO・Virgo・KAGRA
  - L1-19 IceCube とニュートリノ天文学
  - L1-20 一つの現象を複数のメッセンジャーで調べる

---

## LEVEL 2 — 観測する宇宙

### Course 2A: 空の位置・時刻・距離

- Module 2A-1 天球と座標
  - L2-01 地平座標と日周運動
  - L2-02 赤経 RA・赤緯 Dec
  - L2-03 時刻、恒星時、暦、歳差
  - Bridge M2-01 角度・三角比・球面の考え方
- Module 2A-2 距離梯子の最初の段
  - L2-04 年周視差
  - L2-05 標準光源と標準物差し
  - L2-06 距離の誤差とバイアス
  - Bridge M2-02 比例・逆比例・誤差伝播

### Course 2B: 光を測る

- Module 2B-1 光と電磁波
  - L2-07 波長・振動数・光子
  - L2-08 電波からガンマ線まで
  - L2-09 大気の窓と宇宙望遠鏡
  - Bridge P2-01 波・電磁気・量子の入口
- Module 2B-2 測光
  - L2-10 flux・luminosity・逆二乗則
  - L2-11 等級と色指数
  - L2-12 light curve と時間変動
  - Bridge M2-03 指数・対数・関数・グラフ
- Module 2B-3 分光
  - L2-13 連続スペクトルとスペクトル線
  - L2-14 原子が残す吸収線・輝線
  - L2-15 Doppler shift と redshift
  - L2-16 温度・組成・速度を読む
  - Bridge P2-02 原子・電子・エネルギー準位

### Course 2C: 望遠鏡・検出器・データ

- Module 2C-1 光学系
  - L2-17 口径、集光力、焦点距離
  - L2-18 diffraction と angular resolution
  - L2-19 seeing・adaptive optics・干渉計
- Module 2C-2 検出と校正
  - L2-20 CCD・CMOS・電波受信機
  - L2-21 bias・dark・flat 補正
  - L2-22 signal-to-noise ratio
  - L2-23 測定誤差・系統誤差・検出限界
  - Bridge M2-04 平均・標準偏差・正規分布
- Module 2C-3 天文データの形
  - L2-24 天体画像と FITS
  - L2-25 天体カタログと CSV
  - L2-26 metadata・座標照合・選択効果

---

## LEVEL 3 — 太陽・惑星・系外惑星

### Course 3A: 軌道を支配するもの

- Module 3A-1 運動と重力
  - L3-01 位置・速度・加速度
  - L3-02 力、質量、ニュートンの法則
  - L3-03 万有引力、円運動、脱出速度
  - L3-04 ケプラーの三法則
  - Bridge M3-01 ベクトル・微分・面積速度
- Module 3A-2 天体力学の入口
  - L3-05 エネルギーと軌道
  - L3-06 運動量・角運動量
  - L3-07 二体問題、ラグランジュ点、摂動

### Course 3B: 太陽系

- Module 3B-1 形成と構造
  - L3-08 原始惑星系円盤と太陽系形成
  - L3-09 地球型・ガス巨大・氷巨大惑星
  - L3-10 衛星・リング・小惑星・彗星
  - L3-11 カイパーベルト・オールト雲
- Module 3B-2 惑星を比較する
  - L3-12 内部構造と地質活動
  - L3-13 大気・温室効果・気候
  - L3-14 磁気圏と表面環境
  - Bridge P3-01 圧力・気体・熱力学・流体

### Course 3C: 太陽と宇宙天気

- Module 3C-1 最も近い恒星
  - L3-15 太陽の層構造とエネルギー源
  - L3-16 黒点・フレア・コロナ加熱
  - L3-17 太陽風・磁気圏・宇宙天気
  - Bridge P3-02 プラズマと磁場

### Course 3D: 系外惑星と生命

- Module 3D-1 見えない惑星を見つける
  - L3-18 transit 法と light curve
  - L3-19 視線速度法
  - L3-20 直接撮像・位置天文・microlensing
  - L3-21 検出バイアスと惑星人口統計
- Module 3D-2 居住可能性を考える
  - L3-22 habitable zone の意味と限界
  - L3-23 惑星大気の transmission spectroscopy
  - L3-24 biosignature と偽陽性
  - L3-25 アストロバイオロジーと Drake 方程式

---

## LEVEL 4 — 恒星・星間物質

### Course 4A: 恒星を測る

- Module 4A-1 色・温度・明るさ
  - L4-01 黒体放射と Wien の法則
  - L4-02 Stefan–Boltzmann 則と半径
  - L4-03 スペクトル型と化学組成
  - L4-04 HR 図を読む
- Module 4A-2 質量を測る
  - L4-05 連星の種類と軌道
  - L4-06 質量光度関係
  - Bridge M4-01 回帰・最小二乗法・相関

### Course 4B: 恒星の内部と一生

- Module 4B-1 恒星構造
  - L4-07 静水圧平衡
  - L4-08 理想気体・縮退圧・エネルギー輸送
  - L4-09 核融合と元素合成
  - L4-10 恒星構造方程式の見取り図
- Module 4B-2 星形成から主系列へ
  - L4-11 分子雲・Jeans 不安定
  - L4-12 原始星・円盤・ジェット
  - L4-13 主系列星と質量が決める運命
- Module 4B-3 恒星進化
  - L4-14 赤色巨星と漸近巨星分枝
  - L4-15 白色矮星・惑星状星雲
  - L4-16 大質量星・超新星・元素供給

### Course 4C: コンパクト天体と高エネルギー現象

- Module 4C-1 星の残骸
  - L4-17 中性子星・パルサー・マグネター
  - L4-18 ブラックホールと事象の地平面
  - L4-19 降着円盤・ジェット・X 線連星
  - L4-20 Ia / core-collapse 超新星
- Module 4C-2 相対論への橋
  - L4-21 特殊相対論、質量とエネルギー
  - L4-22 時空、重力、Schwarzschild 半径
  - L4-23 重力波を生む連星

### Course 4D: 星間物質

- Module 4D-1 星と銀河をつなぐ物質
  - L4-24 原子・分子・電離ガス・ダスト
  - L4-25 H II 領域と超新星残骸
  - L4-26 星形成と feedback の循環
  - Bridge P4-01 放射輸送・流体・MHD の概念

---

## LEVEL 5 — 銀河・極限宇宙

### Course 5A: 天の川銀河

- Module 5A-1 私たちの銀河を内側から測る
  - L5-01 円盤・バルジ・ハロー・棒構造
  - L5-02 星団・恒星種族・化学進化
  - L5-03 Gaia が描く位置と運動
  - L5-04 銀河中心の超大質量ブラックホール

### Course 5B: 銀河の多様性と進化

- Module 5B-1 銀河を分類し測る
  - L5-05 渦巻・楕円・不規則銀河
  - L5-06 表面輝度・色・スペクトル・質量
  - L5-07 星形成率と金属量
- Module 5B-2 銀河を作り変える過程
  - L5-08 銀河形成と階層的成長
  - L5-09 衝突・合体・潮汐相互作用
  - L5-10 AGN・クエーサー・feedback
  - L5-11 銀河群・銀河団・環境効果

### Course 5C: 見えない質量を推定する

- Module 5C-1 ダークマターの証拠
  - L5-12 銀河回転曲線
  - L5-13 銀河団の運動と高温ガス
  - L5-14 強い・弱い重力レンズ
  - L5-15 CMB・大規模構造との証拠統合
  - Bridge M5-01 行列・固有値・ベイズ推論の入口

### Course 5D: マルチメッセンジャー宇宙

- Module 5D-1 極端な天体現象
  - L5-16 X 線・ガンマ線天文学
  - L5-17 gamma-ray burst と kilonova
  - L5-18 重力波検出器と標準サイレン
  - L5-19 宇宙線・ニュートリノ・IceCube
  - L5-20 マルチメッセンジャー事例 GW170817

---

## LEVEL 6 — 宇宙論

### Course 6A: 膨張する宇宙

- Module 6A-1 距離と膨張
  - L6-01 銀河の redshift と Hubble–Lemaître 則
  - L6-02 宇宙膨張は何が広がることか
  - L6-03 cosmic distance ladder
  - L6-04 標準光源・標準物差し・標準サイレン
  - Bridge M6-01 微分方程式とスケール因子
- Module 6A-2 時空の力学
  - L6-05 等価原理と曲がった時空
  - L6-06 Friedmann 方程式の物理的意味
  - L6-07 臨界密度と宇宙の幾何

### Course 6B: 熱い初期宇宙

- Module 6B-1 Big Bang model の証拠
  - L6-08 熱い初期宇宙というモデル
  - L6-09 Big Bang 元素合成
  - L6-10 再結合と CMB
  - L6-11 CMB 温度ゆらぎと音響ピーク
- Module 6B-2 構造形成
  - L6-12 原始ゆらぎから cosmic web へ
  - L6-13 ダークマターと baryon の成長
  - L6-14 銀河サーベイと BAO

### Course 6C: 現代標準宇宙論と未解決問題

- Module 6C-1 宇宙の成分と歴史
  - L6-15 ΛCDM model
  - L6-16 加速膨張とダークエネルギー
  - L6-17 宇宙年齢・観測可能な宇宙・地平線
  - L6-18 宇宙の未来
- Module 6C-2 未解決問題
  - L6-19 inflation の動機と検証可能性
  - L6-20 dark matter の正体
  - L6-21 dark energy と宇宙定数問題
  - L6-22 Hubble tension と系統誤差
  - L6-23 matter–antimatter asymmetry

---

## LEVEL 7 — 大学天文学・研究実践

### Course 7A: 天体物理の共通言語

- Module 7A-1 力学と連続体
  - L7-01 二体・多体問題と virial theorem
  - L7-02 流体方程式と衝撃波
  - L7-03 plasma・磁気流体 MHD
  - L7-04 Jeans 方程式と銀河力学
- Module 7A-2 物質と放射
  - L7-05 熱力学・統計力学の統合
  - L7-06 原子過程と spectral line
  - L7-07 放射過程: thermal・synchrotron・Compton
  - L7-08 放射輸送方程式と optical depth
  - L7-09 恒星構造方程式の導出と数値解

### Course 7B: 統計・計算・観測データ解析

- Module 7B-1 統計的推論
  - L7-10 確率分布・尤度・最尤法
  - L7-11 回帰・curve fitting・最小二乗法
  - L7-12 confidence interval・hypothesis test
  - L7-13 Bayes theorem・prior・posterior
  - L7-14 selection effect・censoring・systematic error
- Module 7B-2 計算天文学
  - L7-15 浮動小数点・補間・積分・ODE
  - L7-16 Monte Carlo と bootstrap
  - L7-17 N-body simulation
  - L7-18 数値流体・格子法・粒子法の概観
  - L7-19 再現可能性・version control・環境
- Module 7B-3 観測解析
  - L7-20 Python・NumPy・pandas・matplotlib
  - L7-21 Astropy、unit、coordinate、time
  - L7-22 FITS image reduction・photometry
  - L7-23 spectrum reduction・redshift fitting
  - L7-24 time series・periodogram・transit fitting
  - L7-25 catalog・SQL・ADQL・cross-match

### Course 7C: Astronomy Lab

- Module 7C-1 基礎 Python
  - LAB-01 AU と光年を変換する
  - LAB-02 惑星データを表にする
  - LAB-03 惑星軌道をプロットする
- Module 7C-2 恒星と時系列
  - LAB-04 恒星データを散布図にする
  - LAB-05 HR 図を作る
  - LAB-06 light curve を描く
  - LAB-07 transit を検出する
  - LAB-08 系外惑星候補を比較する
- Module 7C-3 カタログ・スペクトル・シミュレーション
  - LAB-09 天体カタログを検索する
  - LAB-10 Gaia データを可視化する
  - LAB-11 spectrum を描く
  - LAB-12 redshift を推定する
  - LAB-13 銀河データを分析する
  - LAB-14 簡単な N-body simulation

### Course 7D: 論文と研究リテラシー

- Module 7D-1 論文を読む
  - L7-26 paper・peer review・preprint
  - L7-27 Title → Abstract → Figures → Conclusion の順で読む
  - L7-28 Introduction・Methods・Results・Discussion
  - L7-29 Figure・Table・error bar・統計的有意性
  - L7-30 arXiv・NASA ADS・引用をたどる
- Module 7D-2 主張を評価する
  - L7-31 model・simulation・observation の区別
  - L7-32 correlation と causation
  - L7-33 observational bias と systematic error
  - L7-34 uncertainty を保った科学コミュニケーション

### Course 7E: Capstone Projects

- Module 7E-1 小規模研究
  - CAP-A 公開系外惑星データから地球に近い特徴を分析する
  - CAP-B 恒星カタログから HR 図と恒星集団を分析する
  - CAP-C 実 light curve から transit 候補を検出する
  - CAP-D 銀河カタログの redshift 分布を分析する
  - CAP-E N-body simulation で惑星軌道を再現する

各 Capstone は目的、問い、前提知識、データ provenance、手順、Python、検証、結果、限界、考察、発展課題、再現用 README を含む。

---

## 学部教育カバレッジ確認

| 大学学部領域 | ASTRAEA |
|---|---|
| 天文学概論・歴史 | L0, L1 |
| 位置天文・時刻・天体力学 | L2A, L3A, L7A |
| 観測・望遠鏡・装置 | L2B–C |
| 数学・基礎物理 | 各 Bridge, L7A–B |
| 太陽・惑星・系外惑星 | L3B–D |
| 恒星・恒星進化・星間物質 | L4A–D |
| コンパクト天体・高エネルギー | L4C, L5D |
| 銀河・銀河団・銀河進化 | L5A–B |
| ダークマター・重力レンズ | L5C |
| 宇宙論・CMB・元素合成 | L6A–C |
| 重力波・ニュートリノ | L1C, L5D |
| 統計・計算・数値 simulation | L7B–C |
| 公開データ解析 | L7B–C |
| 論文読解・研究倫理・不確実性 | L7D |
| 観測実習・課題研究 | LAB, Capstone |

## 参照した大学公式資料

- 東京大学理学部天文学科: https://www.astron.s.u-tokyo.ac.jp/about/undergraduate/
- 京都大学理学部: https://sci.kyoto-u.ac.jp/ja/education/undergraduate/divisions
- Harvard Department of Astronomy: https://astronomy.fas.harvard.edu/concentration-requirements
- University of Arizona Steward Observatory: https://astro.arizona.edu/academics/undergraduate/4-year-plan-astronomy-degree

参照確認日: 2026-08-15
